...
class RCNode:
    ...
    def __lt__(self, other):
        # Float precision handling for priority queue ties
...
class CBSSpaceTimeAStar:
    """Low-level solver for CBS. Only obeys specific constraints given to it."""
    def __init__(self, env: GridEnvironment, battery: BatteryModel):
        self.env = env
        self.battery = battery
        
    def search(self, start, goal, init_theta, v_constraints, e_constraints, critical_soc_limit=0.05, max_time=150):
        open_list = []
        visited = set() 
        
        start_node = RCNode(start[0], start[1], init_theta, 0, 1.0, g_score=0.0)
        
        # Admissible Heuristic based strictly on minimum possible battery drain
        ...
        while open_list:
            ...
            for action, nx, ny, ntheta in transitions:
                ...
                # --- CBS STRICT CONSTRAINT CHECKS ---
                # Check Vertex Constraint (Is this specific tile banned at this specific time?)
                if (nx, ny, ntime) in v_constraints: continue 
                
                # Check Edge Constraint (Am I banned from traversing between these tiles right now?)
                if (current.x, current.y, nx, ny, ntime) in e_constraints: continue
                # ------------------------------------
                ...
                # g(n) is exactly the battery consumed. AI optimizes for physics.
